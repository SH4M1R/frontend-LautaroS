...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API}/api/empleados/login`, {
        method: "POST",
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(825deg, white, red)" }}
    >
      <div
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <div className="alert alert-danger mt-3 text-center py-2">{error}</div>
          )}
        </form>
      </div>
    </div>
